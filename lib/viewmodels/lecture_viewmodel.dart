...
class LectureViewModel with ChangeNotifier {
  ...
  Future<Response> downloadAndSaveLecture(Lecture lecture) async {
    ...
    int indexLecture = _availableLectures[indexPack].children.indexWhere((lecture) => lecture.lesson == lecture.lesson);
    ...
    try {
      ...
      if (oldLectureStatus == LectureStatus.removed) {
        ...
        if (_currentFilter != null && _currentFilter!.isNotEmpty) {
          ...
          int indexLectureFilter = _filteredLectures[indexPackFilter].children.indexWhere((lecture) => lecture.lesson == lecture.lesson);
