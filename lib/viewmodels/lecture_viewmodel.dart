...
class LectureViewModel with ChangeNotifier {
  ...
  Future<Response> downloadAndSaveLecture(Lecture lecture) async {
    ...
    try {
      File lectureFile = await _downloadLectureWithProgress(lecture); // download lecture and save zip temporary as file
  ...
  /// Map containing all lectures that are currently in download, as well as their corresponding download progresses.
  Map<String, double?> lectureDownloadProgress = Map();
  /// Map containing the stream subscriptions of lectures that are currently in download. Subscriptions are cleaned
  /// when the viewmodel is disposed.
  Map<String, StreamSubscription<List<int>>?> lectureDownloadProgressSubscriptions = Map();

  @override
  void dispose() {
    lectureDownloadProgressSubscriptions.forEach((key, value) => value?.cancel());
    super.dispose();
  }

  /// Downloads a lecture as stream and provides proper progress updates.
  Future<File> _downloadLectureWithProgress(Lecture lecture) async {
    _setLectureDownloadProgress(lecture, null);
    List<int> bytes = [];

    final response = await _lectureRepository.downloadLectureAsStream(lecture);

    // Get temporary file
    String dirPath = (await getTemporaryDirectory()).path;
    File file = File('$dirPath/${lecture.fileName}');

    final contentLength = response.contentLength ?? 0;
    lectureDownloadProgressSubscriptions[lecture.fileName] = response.stream.listen((List<int> newBytes) {
      bytes.addAll(newBytes);
      final downloadedLength = bytes.length;
      double progress = downloadedLength / contentLength;
      _setLectureDownloadProgress(lecture, progress);
    }, onError: (e) {
    ...
    // Convert stream subscription to future and wait till stream is closed.
    await lectureDownloadProgressSubscriptions[lecture.fileName]!.asFuture();

    file.writeAsBytesSync(bytes);
    log("Finished downloading lecture");
    _setLectureDownloadProgress(lecture, -1);
    return file;
  }

  /// Updates the download [progress] of the corresponding [lecture] in the [lectureDownloadProgress] map and
  /// calls [notifyListeners].
  _setLectureDownloadProgress(Lecture lecture, double? progress) {
    lectureDownloadProgress.update(lecture.fileName, (value) => value = progress, ifAbsent: () => progress);
    notifyListeners();
  }
